...
function afficherPromotion($promotion){
	...
	function ajouterPromotion($promotion){
		$sql="insert into promo (Referencep,Pourcentage,Categoriesp,Debutpromos,Finpromos) values (:Referencep,:Pourcentage,:Categoriesp,:Debutpromos,:Finpromos)";
	...
	function supprimerPromotions($Referencep){
		$sql="DELETE FROM promo where Referencep= :Referencep";
		$db = config::getConnexion();
        $req=$db->prepare($sql);
		$req->bindValue(':Referencep',$Referencep);
		try{
		...
		$sql="UPDATE promo SET Referencep=:Referencep, Pourcentage=:Pourcentage,Categoriesp=:Categoriesp,Debutpromos=:Debutpromos,Finpromos=:Finpromos WHERE id_promotion=:id_promotion";
...
try{		
  ...
  print_r($datas);
        ...
        $sql="SELECT * from promo where id_promotion=$id_promotion";
        $db = config::getConnexion();
        try{
        $liste=$db->query($sql);
        return $liste;
